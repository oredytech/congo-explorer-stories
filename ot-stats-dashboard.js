...
// JavaScript pour le tableau de bord OT Site Stats
document.addEventListener("DOMContentLoaded", function() {
    ...
    function updateChartData(period, startDate = null, endDate = null) {
        ...
        let url = `${ajaxurl}?action=ot_stats_range&period=${period}`;
    ...
    function loadDashboardData() {
        ...
        fetch(`${ajaxurl}?action=ot_stats_summary`)
            .then(response => response.json())
            .then(data => {
        ...
        fetch(`${ajaxurl}?action=ot_stats_countries`)
            .then(response => response.json())
            .then(data => {
        ...
        fetch(`${ajaxurl}?action=ot_stats_hourly`)
            .then(response => response.json())
            .then(data => {
                ...
                row.innerHTML = `
                    ...
                    <td>
                        <div class="ot-progress-bar">
                            <div class="ot-progress-fill" style="width: ${(country.views / data.countries[0].views) * 100}%"></div>
                        </div>
                    </td>
                `;
    ...
    document.querySelector('.ot-period-btn[data-period="week"]')?.click();
});

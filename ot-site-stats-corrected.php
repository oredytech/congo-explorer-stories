...
add_action('admin_enqueue_scripts', function($hook) {
    if (strpos($hook, 'ot-site-stats') !== false) {
        ...
        wp_enqueue_style('ot-stats-admin', plugin_dir_url(__FILE__) . 'ot-site-stats-admin.css', [], '2.1');
        wp_enqueue_script('ot-stats-dashboard', plugin_dir_url(__FILE__) . 'ot-stats-dashboard.js', ['chart-js'], '2.1', true);
...
function ot_track_view($request) {
    ...
    // Géolocalisation avec gestion d'erreur
    $country = 'Inconnu';
    try {
        $geo_response = @file_get_contents("http://ip-api.com/json/{$ip}?timeout=5");
    ...
    $result = $wpdb->insert($wpdb->prefix . 'ot_stats', [
        'post_id' => $post_id,
        'ip_address' => $ip,
        'user_agent' => $agent,
        'country' => $country,
...
function ot_stats_settings_page() {
    ...
    <div class="ot-stats-wrap">
        <div class="ot-stats-header">
            <h1>⚙️ Paramètres OT Site Stats</h1>
            <p class="ot-stats-subtitle">Configuration générale du plugin de statistiques</p>
        </div>
        
        <div class="ot-settings-container">
            <div class="ot-stats-card">
                ...
                <form method="post" action="options.php">
                    ...
                    <div class="ot-form-group">
                        <label for="ot_tsx_site_url" class="ot-label">🌐 URL du site TSX</label>
                        <input type="url" 
                               name="ot_tsx_site_url" 
                               id="ot_tsx_site_url"
                               value="<?php echo esc_attr(get_option('ot_tsx_site_url', '')); ?>" 
                               class="ot-input"
                               placeholder="https://monsite.com" />
                        <p class="ot-description">URL complète de votre site frontend TSX</p>
                    </div>
                    
                    <div class="ot-form-group">
                        <label for="ot_stats_default_views" class="ot-label">👁️ Vues par défaut</label>
                        <input type="number" 
                               name="ot_stats_default_views" 
                               id="ot_stats_default_views"
                               value="<?php echo esc_attr(get_option('ot_stats_default_views', '0')); ?>" 
                               min="0" 
                               class="ot-input-number" />
                        <p class="ot-description">Nombre de vues affiché par défaut pour les nouveaux articles</p>
                    </div>
                    
                    <div class="ot-form-actions">
                        <?php submit_button('💾 Enregistrer les paramètres', 'primary', 'submit', false, ['class' => 'ot-btn-primary']); ?>
                    </div>
                </form>
            </div>
            ...
            <div class="ot-stats-card">
                ...
                <div class="ot-info-grid">
                    <div class="ot-info-item">
                        ...
                        <span class="ot-info-value">2.1</span>
                    </div>
                    <div class="ot-info-item">
                        <span class="ot-info-label">Développeur</span>
                        <span class="ot-info-value">Oredy Technologies</span>
                    </div>
                    <div class="ot-info-item">
                        <span class="ot-info-label">Base de données</span>
                        <span class="ot-info-value ot-status-active">✅ Active</span>
                    </div>
                </div>
            </div>
        </div>
...
// --- Nouvelles fonctions AJAX corrigées ---
...
function ot_stats_range_ajax() {
    ...
    $sql = "SELECT $format as label, COUNT(*) as views 
            FROM $table 
            WHERE $where 
            GROUP BY $group 
...
function ot_stats_countries_ajax() {
    ...
    $countries = $wpdb->get_results("
        ...
        FROM $table 
        WHERE country IS NOT NULL AND country != 'Inconnu' AND country != ''
        GROUP BY country 
        ORDER BY views DESC 
        LIMIT 10
    ");
...
// --- Page admin principale corrigée ---
function ot_stats_admin_page() {
    ...
    $today_views = $wpdb->get_var("SELECT COUNT(*) FROM $table WHERE DATE(view_date) = CURDATE()");
    $unique_visitors = $wpdb->get_var("SELECT COUNT(DISTINCT ip_address) FROM $table WHERE view_date >= CURDATE() - INTERVAL 30 DAY");
    $top_countries = $wpdb->get_results("SELECT country, COUNT(*) as views FROM $table WHERE country IS NOT NULL AND country != 'Inconnu' AND country != '' GROUP BY country ORDER BY views DESC LIMIT 5");
            ...
            <table class="ot-table-modern">
                ...
                <tbody>
                    <?php foreach ($top_articles as $index => $article): 
                        $title = get_the_title($article->post_id) ?: 'Article supprimé';
                        $max_views = $top_articles[0]->views ?? 1;
                    ?>
                    <tr>
                        <td><?php echo $index + 1; ?></td>
                        <td>
                            <?php if (get_post_status($article->post_id)): ?>
                                <a href="<?php echo get_permalink($article->post_id); ?>" target="_blank">
                                    <?php echo esc_html($title); ?>
                                </a>
                            <?php else: ?>
                                <span class="ot-deleted-post"><?php echo esc_html($title); ?></span>
                            <?php endif; ?>
                        </td>
                        <td><?php echo number_format($article->views); ?></td>
                        <td>
                            <div class="ot-progress-bar">
                                <div class="ot-progress-fill" style="width: <?php echo ($article->views / $max_views) * 100; ?>%"></div>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; ?>
            ...
            <table class="ot-table-modern ot-country-table">
                ...
                <tbody>
                    <?php foreach ($top_countries as $index => $country): 
                        $max_country_views = $top_countries[0]->views ?? 1;
                    ?>
                    <tr>
                        <td><?php echo $index + 1; ?></td>
                        ...
                        <td>
                            <div class="ot-progress-bar">
                                <div class="ot-progress-fill" style="width: <?php echo ($country->views / $max_country_views) * 100; ?>%"></div>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; ?>
    ...
    <!-- Scripts pour la carte corrigés -->
        ...
        function drawGeoChart() {
            var data = google.visualization.arrayToDataTable([
                ...
                foreach ($top_countries as $country) {
                    if ($country->country && strtolower($country->country) !== 'inconnu' && $country->country !== '') {
            ...
            if (data.getNumberOfRows() > 1) {
                ...
                document.getElementById("ot-stats-map").innerHTML = '<div style="text-align:center; padding:50px; color:#666;">Pas encore de données géographiques disponibles</div>';
            }
        }
    </script>
    <?php
